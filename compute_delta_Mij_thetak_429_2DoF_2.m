function [delta_Mij_thetak_429] = compute_delta_Mij_thetak_429_2DoF_2(xi_ai, exp_ai, Pi, gsli0, Mi_b, i, j, k)
...
c_j = j;
        l = max(c_i,c_j);
        for add=l:n_Dof
            [Aki] = compute_Aij_for_Ji_427_2DoF(exp_ai, Pi, k, c_i);
            [Alk] = compute_Aij_for_Ji_427_2DoF(exp_ai, Pi, add, k);
            [Alj] = compute_Aij_for_Ji_427_2DoF(exp_ai, Pi, add, c_j);
            Lie_Br_1 = liebracket_426_new(Aki*xi_ai(:,c_i), xi_ai(:,k));
            
            [Ali] = compute_Aij_for_Ji_427_2DoF(exp_ai, Pi, add, c_i);
            [Akj] = compute_Aij_for_Ji_427_2DoF(exp_ai, Pi, k, c_j);
